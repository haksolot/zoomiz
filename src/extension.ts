...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('zoomiz.extendSelection', () => {
        startZoomiz('extend');
    }));
}

function startZoomiz(mode: 'jump' | 'extend') {
    ...
    const extendWhenSelected = config.get<boolean>('extendWhenSelected', false);

    // Auto-switch to extend mode if configured and selection exists
    if (mode === 'jump' && extendWhenSelected && !editor.selection.isEmpty) {
        mode = 'extend';
    }

    // For extend mode, we keep the original anchor.
    // If in jump mode, the anchor will move with the cursor.
    const initialAnchor = editor.selection.anchor;
    ...
    inputBox.title = mode === 'extend' ? "Zoomiz: Extend Selection" : "Zoomiz: Jump";
    ...
    inputBox.onDidChangeValue((value) => {
        ...
        if (isJump) {
            ...
            if (target) {
                executeJump(editor, target, mode, initialAnchor);
                inputBox.hide();
                return;
            }
        }
...
function executeJump(editor: vscode.TextEditor, target: JumpTarget, mode: 'jump' | 'extend', initialAnchor: vscode.Position) {
    if (mode === 'extend') {
        // Extend selection to include the target range
        editor.selection = new vscode.Selection(initialAnchor, target.range.end);
    } else {
        // Just jump to the start of the target
        const newActive = target.range.start;
        editor.selection = new vscode.Selection(newActive, newActive);
    }
